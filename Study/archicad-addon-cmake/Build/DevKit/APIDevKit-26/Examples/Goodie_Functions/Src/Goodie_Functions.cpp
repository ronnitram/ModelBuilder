...
static void		Do_SplineApprox (void)
{
	...
	err = ACAPI_Interface (APIIo_GetPolyID, &pline);
	if (err != NoError) {
...
static void	Do_DeleteAutotext (void)
{
	...
	if (err == NoError && !autoTexts.IsEmpty ()) {
		...
		err = ACAPI_Goodies (APIAny_DeleteAnAutoTextID, (void*)((const char*) dbKey.ToCStr ()));
		if (err != NoError)
...
static void	Do_SetAutotexts (void)
{
	...
	GSErrCode	err = ACAPI_Goodies (APIAny_SetAnAutoTextID, &dbKey);
	if (err == NoError) {
		Do_GetAutotexts (&dbKey);
	} else {
	...
	err = ACAPI_Goodies (APIAny_SetAnAutoTextID);
	if (err == NoError) {
		Do_GetAutotexts (&dbKey);
	} else {
...
static void Do_DownloadWeatherData (void)
{
	...
	IO::URI::URI connectionUrl ("http://23.23.243.100/");
...
static void Do_GetRoomImage (void)
{
	...
	UserItemDialog dialog (3, 400, 400, [&] (const DG::UserItem& userItem, const UIndex index) {
		...
		ACAPI_Goodies (APIAny_GetRoomImageID, &image);
