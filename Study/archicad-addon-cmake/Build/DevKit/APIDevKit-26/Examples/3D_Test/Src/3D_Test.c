...
API_MaterialType	GetUMaterial (API_AttributeIndex iumat)
{
	static GS::HashTable<API_AttributeIndex, API_MaterialType> umatCache;
...
static GSErrCode	Do_Dump3DModel (bool fromSaveAs)
{
	...
	if (!fromSaveAs) {		// we must switch to the 3D Window's sight
		err = ACAPI_3D_SelectSight (nullptr, &origSight);
		if (err != NoError) {
