...
static API_Guid AddRandomClassification (API_ClassificationSystem& systemOut)
{
	...
	GSDateRecord gsDateRecord (gsTimeRecord.year, gsTimeRecord.month, gsTimeRecord.day);

	systemOut.editionDate = gsDateRecord;
...
**------------------------------------------------------------------------------*/
static void TestPropertiesOnAttribute ()
{
...
static void TestPropertiesOnElemList (const API_Guid& elemGuid)
{
	...
	// Create the property definition and make if available for all of the specified elems
	ASSERT_NO_ERROR (ACAPI_ElementList_AddProperty (definition, elemList));
	...
	// ACAPI_ElementList_AddProperty can be used to add to the availability list of an existing property definition
	ASSERT_NO_ERROR (ACAPI_ElementList_AddProperty (definition, elemList));

	ASSERT_NO_ERROR (ACAPI_Element_GetPropertyDefinitions (elemGuid, API_PropertyDefinitionFilter_UserDefined, elemProperties));
	ASSERT (elemProperties.Contains (definition));

	// ACAPI_ElementList_ModifyPropertyValue can be used the change the property value of all elements in the list
	API_Property property;
	property.definition = definition;
	property.value = definition.defaultValue.basicValue;
	property.value.singleVariant.variant.intValue = 42;
	ASSERT_NO_ERROR (ACAPI_ElementList_ModifyPropertyValue (property, elemList));

	API_Property propertyResult;
	ASSERT_NO_ERROR (ACAPI_Element_GetPropertyValue (elemGuid, definition.guid, propertyResult));
	ASSERT (propertyResult == property);

	// ACAPI_ElementList_DeleteProperty makes the property unavailable for the given elements
	ASSERT_NO_ERROR (ACAPI_ElementList_DeleteProperty (definition.guid, elemList));
	ASSERT_NO_ERROR (ACAPI_Property_GetPropertyDefinition (definition));
	ASSERT (definition.availability.IsEmpty ());
...
static void TestPropertiesWithExpressionsWithCycles ()
{
	...
	ACAPI_Goodies (APIAny_GetPropertyExprReferenceStringID, &definition1, &definition1expr);

	GS::UniString definition2expr; 
	ACAPI_Goodies (APIAny_GetPropertyExprReferenceStringID, &definition2, &definition2expr);
...
static void TestXMLImport ()
{
	...
	ASSERT_NO_ERROR (ACAPI_Goodies (APIAny_GetPropertyExprReferenceStringID, &importedDefinition2, &propertyReferenceString));
...
**----------------------------------------------------------------*/
static void RunTestsOnSelectedElem (const API_Guid& elemGuid)
{
...
static void ListPropertiesWithIntegerDataType (const API_Guid& elemGuid)
{
	...
	if (string.IsEmpty ()) {
		DGAlert (DG_INFORMATION, "Properties with integer value for the selected element", "No properties with integer value found.", "", "Ok");
	} else {
		DGAlert (DG_INFORMATION, "Properties with integer value for the selected element", "", string, "Ok");
	}
}
...
**----------------------------------------------------------------------------*/
static void ListVisibleProperties (const API_Guid& elemGuid, API_PropertyDefinitionFilter filter)
{
	GS::Array<API_PropertyDefinition> definitions;
	...
	for (UInt32 i = 0; i < properties.GetSize (); i++) {
		...
		const GSErrCode error = PropertyUtils::PropertyToString (properties[i], propertyValue);
		if (error == NoError) {
...
{
	ListVisibleProperties (elemGuid, API_PropertyDefinitionFilter_UserDefined);
}


static void ListVisibleFundamentalBuiltInProperties (const API_Guid& elemGuid)
{
	ListVisibleProperties (elemGuid, API_PropertyDefinitionFilter_FundamentalBuiltIn);
}


static void ListVisibleUserLevelBuiltInProperties (const API_Guid& elemGuid)
{
	ListVisibleProperties (elemGuid, API_PropertyDefinitionFilter_UserLevelBuiltIn);
}
...
**----------------------------------------------------------*/
static void DeleteIntegerPropeties ()
{
	GS::Array<API_PropertyDefinition> definitions;
	ASSERT_NO_ERROR (ACAPI_Property_GetPropertyDefinitions (APINULLGuid, definitions));
	GS::Array<API_Guid> selectedElements = PropertyTestHelpers::GetSelectedElements();
...
static GSErrCode	__ACENV_CALL	NotificationCallback (const API_NotifyElementType *elemType)
{
...
static GSErrCode ListAllProperties (const GS::UniString& alertTitle, const std::function<bool (const API_PropertyDefinition&)>& condition)
{
	GS::Array<API_PropertyDefinition> definitions;
	ASSERT_NO_ERROR (ACAPI_Property_GetPropertyDefinitions (APINULLGuid, definitions));
	...
	DGAlert (DG_INFORMATION, alertTitle, string, "", "Ok");
...
{
	return ListAllProperties ("All Built-in Properties", [&] (const API_PropertyDefinition& definition) {
		return definition.definitionType != API_PropertyCustomDefinitionType;
	});
}
...
{
	return ListAllProperties ("All Custom Properties", [&] (const API_PropertyDefinition& definition) {
		return definition.definitionType == API_PropertyCustomDefinitionType;
	});
}


static GSErrCode ListFirstProperty (const GS::UniString& alertTitle, const std::function<bool (const API_PropertyDefinition&)>& condition)
{
	GS::Array<API_PropertyDefinition> definitions;
	ASSERT_NO_ERROR (ACAPI_Property_GetPropertyDefinitions (APINULLGuid, definitions));

	GS::UniString string;
	UIndex firstMatchingDefinitionIndex = definitions.FindFirst (condition);
	if (firstMatchingDefinitionIndex != MaxUIndex) {
		API_PropertyDefinition definition;
		definition.guid = definitions[firstMatchingDefinitionIndex].guid;
		ASSERT_NO_ERROR (ACAPI_Property_GetPropertyDefinition (definition));
		string = definition.name;
	}
...
static GSErrCode ListFirstBuiltInProperty ()
{
	return ListFirstProperty ("First Built-in Property", [&] (const API_PropertyDefinition& definition) {
		return definition.definitionType != API_PropertyCustomDefinitionType;
	});
}


static GSErrCode ListFirstCustomProperty ()
{
	return ListFirstProperty ("First Built-in Property", [&] (const API_PropertyDefinition& definition) {
		return definition.definitionType == API_PropertyCustomDefinitionType;
	});
...
static GSErrCode ListVisiblePropertiesOfCurrentDefaultElem (API_PropertyDefinitionFilter filter)
{
	API_ToolBoxItem tboxInfo;
	BNZeroMemory (&tboxInfo, sizeof (API_ToolBoxItem));
	GSErrCode errCode = ACAPI_Environment (APIEnv_GetToolBoxModeID, &tboxInfo, nullptr);
	if (errCode != NoError) {
		return errCode;
	}
	...
	DGAlert (DG_INFORMATION, "Properties of the current default elem", string, "", "Ok");
...
{
	return ListVisiblePropertiesOfCurrentDefaultElem (API_PropertyDefinitionFilter_UserDefined);
}


static GSErrCode ListVisibleFundamentalBuiltInPropertiesOfCurrentDefaultElem ()
{
	return ListVisiblePropertiesOfCurrentDefaultElem (API_PropertyDefinitionFilter_FundamentalBuiltIn);
}


static GSErrCode ListVisibleUserLevelBuiltInPropertiesOfCurrentDefaultElem ()
{
	return ListVisiblePropertiesOfCurrentDefaultElem (API_PropertyDefinitionFilter_UserLevelBuiltIn);
}
			...
			[&] () -> GSErrCode {
				try {
					switch (menuParams->menuItemRef.itemIndex) {
						...
						case 11: SelectionProperties::DeleteIntegerPropeties (); break;
						case 12: break; // "-"
						case 13: SelectionProperties::SetAllStringPropertiesToAbcd (); break;
						case 14: SetAllStringPropertiesToAbcdForCurrentDefaultElem (); break;
						case 15: SetAllStringPropertiesToAbcdForCurrentDefaultElem_ExampleForDocumentation (); break;
						case 16: break; // "-"
						case 17: ListAllBuiltInProperties (); break;
						case 18: ListAllCustomProperties (); break;
						case 19: ListFirstBuiltInProperty (); break;
						case 20: ListFirstCustomProperty (); break;
						case 21: PropertyTestHelpers::CallOnAllSelectedElems (ListPropertiesWithIntegerDataType); break;
						case 22: break; // "-"
						case 23: ListAllBuiltInPropertyGroups (); break;
						case 24: ListAllCustomPropertyGroups (); break;
						case 25: ListAllPropertiesOfFirstBuiltInGroup (); break;
						case 26: ListAllPropertiesOfFirstCustomGroup (); break;
						case 27: break; // "-"
						case 28: PropertyTestHelpers::CallOnSelectedElem (ListVisibleUserDefinedProperties); break;
						case 29: PropertyTestHelpers::CallOnSelectedElem (ListVisibleFundamentalBuiltInProperties); break;
						case 30: PropertyTestHelpers::CallOnSelectedElem (ListVisibleUserLevelBuiltInProperties); break;
						case 31: ListVisibleUserDefinedPropertiesOfCurrentDefaultElem (); break;
						case 32: ListVisibleFundamentalBuiltInPropertiesOfCurrentDefaultElem (); break;
						case 33: ListVisibleUserLevelBuiltInPropertiesOfCurrentDefaultElem (); break;
						case 34: break; // "-"
						case 35: ExportAllBuiltInPropertiesToXMLFile (); break;
						case 36: break; // "-"
						case 37: PropertyTestHelpers::CallOnSelectedElem (WatchElement); break;
						case 38: PropertyTestHelpers::CallOnSelectedElem (UnwatchElement); break;
						case 39: break; // "-"
						case 40: PropertyTestHelpers::CallOnAllSelectedElems (RunTestsOnSelectedElem); break;
						case 41: break; // "-"
						case 42: TestPropertiesOnAttribute (); break;
						case 43: break; // "-"
						case 44: PropertyTestHelpers::CallOnAllSelectedElems (SavePropertyValuesOfElement); break;
						case 45: PropertyTestHelpers::CallOnAllSelectedElems (LoadPropertyValuesOfElement); break;
...
GSErrCode	__ACENV_CALL Initialize	(void)
{
	...
	if (err == NoError) {
		err = ACAPI_Notify_InstallElementObserver (NotificationCallback);
	}
