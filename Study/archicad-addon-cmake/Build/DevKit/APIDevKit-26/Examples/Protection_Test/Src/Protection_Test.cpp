...
{
	return ACAPI_Protection_GetPartnerId () == "GRAPHISOFT SE";
}


static void	Command_DisplayProtectionInfo (void)
{
	const GS::UniString isNetworkLicense = ACAPI_Protection_GetProtectionMode () & APIPROT_NET_MASK ? "Yes" : "No";
	const GS::UniString serialNumber = GS::ValueToUniString (ACAPI_Protection_GetSerialNumber ());
...
API_AddonType	__ACENV_CALL	CheckEnvironment (API_EnvirParams* envir)
{
	...
	//   just demo mode
	if (ACAPI_Protection_GetProtectionMode () & APIPROT_DEMO_MASK)
...
GSErrCode	__ACENV_CALL	RegisterInterface (void)
{
	...
	err = ACAPI_Register_Menu (PROTECTIONTEST_MENU_STRINGSID, PROTECTIONTEST_PROMPT_STRINGSID, MenuCode_UserDef, MenuFlag_Default);
	if (err != NoError)
		DBPrintf ("Protection_Test:: RegisterInterface() ACAPI_Register_Menu failed\n");
...
{
	GSErrCode err = ACAPI_Install_MenuHandler (PROTECTIONTEST_MENU_STRINGSID, MenuCommandHandler);
	if (err != NoError)
		DBPrintf ("Protection_Test:: Initialize () ACAPI_Install_MenuHandler failed\n");
