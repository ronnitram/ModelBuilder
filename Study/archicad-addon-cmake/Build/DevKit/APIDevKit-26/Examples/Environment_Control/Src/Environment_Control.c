...
const API_EnvironmentID ApiEnvSetPreferencesID = static_cast<API_EnvironmentID> ('SPRF');
...
static void		Do_TestEnvironmentFunctions (void)
{
...

	{
		...
		err = ACAPI_Environment (APIEnv_ResetLibrariesID);
		if (err != NoError)
			WriteReport_Alert ("Error");
		...
		char guidStr[512];
		ACAPI_Environment (APIEnv_GetLibPartUnIDStrID, &libLocation, &guidStr);
	}
	{
	...
	{
		Int32 num = 0;
		GSErrCode err = ACAPI_Navigator (APINavigator_GetNavigatorSetNumID, &num);
		if (err != NoError)
			WriteReport_Alert ("Error");
	}
	{
		...
		ACAPI_Navigator (APINavigator_GetNavigatorParentItemID, &navigatorItemGuid, &navigatorItem);
	}
	{
		API_NavigatorItem navigatorItem;
		GS::Array<API_NavigatorItem> foundItems;
		ACAPI_Navigator (APINavigator_SearchNavigatorItemID, &navigatorItem, nullptr, &foundItems);
	}
	{
		API_AttributeIndex layerCombInd = 3;
	...
	}
	{
		...
		GSErrCode err = ACAPI_Environment (APIEnv_IsInStairEditModeID, &isInStairEditMode);
		if (err != NoError)
			WriteReport_Alert ("Error");
	}
	{
	...
	{		
		{
			...
			err = ACAPI_Environment (ApiEnvSetPreferencesID, &unitPrefs, (void *) APIPrefs_WorkingUnitsID);
			if (err != NoError)
				WriteReport_Alert ("Error");

			err = ACAPI_Environment (ApiEnvSetPreferencesID, &oldUnitPrefs, (void *) APIPrefs_WorkingUnitsID);
			if (err != NoError)
				WriteReport_Alert ("Error");
		}
		{
			API_CalcUnitPrefs			unitPrefs;
			GSErrCode err = ACAPI_Environment (APIEnv_GetPreferencesID, &unitPrefs, (void *)APIPrefs_CalcUnitsID);
			if (err != NoError)
				WriteReport_Alert ("Error");
			...
			err = ACAPI_Environment (ApiEnvSetPreferencesID, &unitPrefs, (void *)APIPrefs_CalcUnitsID);
			if (err != NoError)
				WriteReport_Alert ("Error");
		...
		}
		{
			...
			err = ACAPI_Environment (ApiEnvSetPreferencesID, &dimPrefs, (void *) APIPrefs_DimensionsID);
			if (err != NoError)
				WriteReport_Alert ("Error");
		...
		}
		{
			...
			GSErrCode err = ACAPI_Environment (APIEnv_GetPreferencesID, &calcRulesPrefs, reinterpret_cast<void*> (APIPrefs_CalcRulesID));
			if (err != NoError)
				WriteReport_Alert ("Error");
			...
			err = ACAPI_Environment (ApiEnvSetPreferencesID, &invalidCalcRulesPrefs, reinterpret_cast<void*> (APIPrefs_CalcRulesID));
			...
			err = ACAPI_Environment (ApiEnvSetPreferencesID, &validCalcRulesPrefs, reinterpret_cast<void*> (APIPrefs_CalcRulesID));
			if (err != NoError)
				WriteReport_Alert ("Error");

			err = ACAPI_Environment (ApiEnvSetPreferencesID, &calcRulesPrefs, reinterpret_cast<void*> (APIPrefs_CalcRulesID));
			if (err != NoError)
				WriteReport_Alert ("Error");
		}
		{
	...
	}
	{
		GSErrCode commandErrorCode = ACAPI_CallCommand (GS::UniString ("ChangeProjectNotesAndCreateGhostRecordCommand"), [&] () ->GSErrCode {
			...
			err = ACAPI_Environment (APIEnv_ChangeProjectNotesID, &noteInfo, nullptr);
			if (err != NoError)
				return err;
			...
			err = ACAPI_Database (APIDb_GetElevationDatabasesID, &dbases, nullptr);
			if (err == NoError)
				nDbases = BMpGetSize (reinterpret_cast<GSPtr>(dbases)) / Sizeof32 (API_DatabaseUnId);

			if (err != NoError || nDbases == 0) {
				err = ACAPI_Database (APIDb_GetLayoutDatabasesID, &dbases, nullptr);
				nDbases = BMpGetSize (reinterpret_cast<GSPtr>(dbases)) / Sizeof32 (API_DatabaseUnId);
			}
			...
			if (err == NoError && nDbases > 0) {
				...
				err = ACAPI_Environment (APIEnv_CreateGhostRecordID, &apiGhostRecord, nullptr);
			}
...
static void		Do_GetServerApplication (void)
{
	...
	ACAPI_Environment (APIEnv_ApplicationID, &serverApp, nullptr);
...
static void	Do_DumpPreferences (void)
{
	...
	err = NoError;
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &unitPrefs, reinterpret_cast<void*> (APIPrefs_WorkingUnitsID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &dimPrefs, reinterpret_cast<void*> (APIPrefs_DimensionsID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &legacyPrefs, reinterpret_cast<void*> (APIPrefs_LegacyID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &zonePrefs, reinterpret_cast<void*> (APIPrefs_ZonesID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &safetyPrefs, reinterpret_cast<void*> (APIPrefs_DataSafetyID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &calcUnitPrefs, reinterpret_cast<void*> (APIPrefs_CalcUnitsID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &calcRulesPrefs, reinterpret_cast<void*> (APIPrefs_CalcRulesID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &refLevelsPrefs, reinterpret_cast<void*> (APIPrefs_ReferenceLevelsID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &imagingAndCalcPrefs, reinterpret_cast<void*> (APIPrefs_ImagingAndCalcID));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &fpCutDefPrefs, reinterpret_cast<void*> (APIPrefs_FloorPlanCutPlaneDef));
	err |= ACAPI_Environment (APIEnv_GetPreferencesID, &layoutsPrefs, reinterpret_cast<void*> (APIPrefs_LayoutsID));
	if (err != NoError) {
	...
	WriteReport ("    Wall insulation skins: %s", arrayToString (calcRulesPrefs.wallInsulationFills, GS::ValueToUniString<API_AttributeIndex>).ToCStr ().Get ());
	WriteReport ("    Wall air skins: %s", arrayToString (calcRulesPrefs.wallAirFills, GS::ValueToUniString<API_AttributeIndex>).ToCStr ().Get ());
	WriteReport ("    Shell insulation skins: %s", arrayToString (calcRulesPrefs.shellInsulationFills, GS::ValueToUniString<API_AttributeIndex>).ToCStr ().Get ());
	WriteReport ("    Roof insulation skins: %s", arrayToString (calcRulesPrefs.roofInsulationFills, GS::ValueToUniString<API_AttributeIndex>).ToCStr ().Get ());
...
static void Do_Dump3DProjection (void)
{
	...
	err = ACAPI_Environment (APIEnv_Get3DProjectionSetsID, &p, nullptr);
	if (err != NoError) {
...
static void Do_DumpDocumentFrom3DDefaults (void)
{
	...
	GSErrCode err = ACAPI_Environment (APIEnv_GetDocumentFrom3DDefaultsID, &d3Default, nullptr);
	if (err != NoError) {
...
static void		Do_ChangeGhostRecord (void)
{
	...
	err = ACAPI_Environment (APIEnv_ChangeGhostRecordID, &apiGhostRecord, nullptr);
	if (err != NoError) {
...
static void		Do_DeleteGhostRecord (void)
{
	...
	err = ACAPI_Environment (APIEnv_DeleteGhostRecordID, &apiGhostRecord, nullptr);
	if (err != NoError) {
...
static void		Do_Hide (void)
{
	...
	GSErrCode err = ACAPI_Environment (APIEnv_ChangeShowHideStateID, &toShow, nullptr);
...
static void		Do_SwitchToArrow (void)
{
	...
	if (ACAPI_Environment (APIEnv_SetToolBoxModeID, &tbi, nullptr) == NoError) {
...
static void		Do_ListLoadedLibraries (void)
{
	...
	if (ACAPI_Environment (APIEnv_GetLibrariesID, &libInfo, nullptr) == NoError) {
...
static void		Do_NewLibraryDialog (void)
{
	...
	GSErrCode err = ACAPI_Environment (APIEnv_NewLibDialID, defName, &newLib);
	if (err == NoError) {
		GS::UniString	infoString;
		GS::UniString	libPath = newLib.ToDisplayText ();

		err = ACAPI_Environment (APIEnv_ResetLibrariesID);					// remove all local and server libraries
		if (err == NoError) {
			err = ACAPI_Environment (APIEnv_AddLibrariesID, &newLib);		// add the selected local library to the active libraries
			if (err == NoError)
...
static void		Do_AddImageFilesToLibrary (void)
{
	...
	if (DBVERIFY (ACAPI_Environment (APIEnv_GetSpecFolderID, &specID, &searchFolderLocation) == NoError))
	...
	GS::Array<API_LibraryInfo> libInfo;
	if (ACAPI_Environment (APIEnv_GetLibrariesID, &libInfo) == NoError && libInfo.GetSize () > 0) {
	...
	if (DBVERIFY (!targetFolderLocation.IsEmpty () && IO::fileSystem.IsLegal (targetFolderLocation))) {
		...
		GSErrCode err = ACAPI_Environment (APIEnv_CreateFolderInLibraryID, &targetFolderLocation);
		if (err == NoError) {
...
static void		Do_ChangeNavigatorItem (void)
{
	...
	err = ACAPI_Navigator (APINavigator_GetNavigatorChildrenItemsID, &item, nullptr, &items);
	if (err != NoError)
		return;
	...
	for (auto& childItem : items) {
		if (childItem.itemType == API_StoryNavItem) {
			...
			// settings
			// layer combination
			API_AttributeIndex attributeCount, i;
			...
			for (i = attributeCount; i > 0; --i) {
				attribute.header.index = i;
				err = ACAPI_Attribute_Get (&attribute);
				API_Attribute modif = attribute;
				err = ACAPI_Attribute_Modify (&modif, nullptr);
				if (err == NoError) {
			...
			err = ACAPI_Navigator (APINavigator_GetNavigatorViewID, &childItem, &oldView);
			if (err == NoError) {
			...
			err = ACAPI_Navigator (APINavigator_ChangeNavigatorViewID, &childItem, &view);
		} else if (childItem.itemType == API_FolderNavItem) {
			// change settings
			// name
			childItem.customName = true;
			GS::UCopy (L ("Name from API"), childItem.uName);
			err = ACAPI_Navigator (APINavigator_ChangeNavigatorViewID, &childItem, nullptr);
		}
	}
}
...
static void Do_ChangeDocumentFrom3DDefaults (void)
{
	...
	err = ACAPI_Environment (APIEnv_ChangeDocumentFrom3DDefaultsID, &d3Default, nullptr);
	if (err != NoError) {
...
static void Do_DumpPartialStructureDisplay (void)
{
	...
	GSErrCode err = ACAPI_Environment (APIEnv_GetStructureDisplayID, &structureDisplay, nullptr);
	if (err == NoError) {
		char state[256];
...
static void Do_ChangePartialStructureDisplay (void)
{
	...
	err = ACAPI_Environment (APIEnv_ChangeStructureDisplayID, &structureDisplay, nullptr);
	if (err != NoError) {
...
static void Do_ChangeModelViewOptions (void)
{
	...
	err = ACAPI_Environment (APIEnv_GetViewOptionsID, &viewOptions, nullptr);
	if (err == NoError) {
	...
	short ind = 3;
	err = ACAPI_Environment (APIEnv_ChangeViewOptionsID, &viewOptions, &ind);
	if (err != NoError) {
...
static void		Do_GetConvertedUnitValue (void)
{
	...
	DBASSERT (ACAPI_Environment (APIEnv_GetConvertedUnitValueID, &data) == NoError);
...
static GSErrCode	UnlockSurveyPoint ()
{
	...
	if (response == DG_OK) {
		bool isSurveyPointLocked = false;
		return ACAPI_Environment (APIEnv_SetSurveyPointLockedID, &isSurveyPointLocked);
	}
...
static void		Do_ChangeGeoLocation (void)
{
	...
	err = ACAPI_CallUndoableCommand ("Change GeoLocation from API", [&] () -> GSErrCode {
		GSErrCode errCode = ACAPI_Environment (APIEnv_SetGeoLocationID, &geoLocation);
		if (errCode == APIERR_NOTEDITABLE && IsSurveyPointLocked ()) {
			if (UnlockSurveyPoint () == NoError)
				errCode = ACAPI_Environment (APIEnv_SetGeoLocationID, &geoLocation);
		}
		return errCode;
	});
	...
	if (err == NoError) {
		...
		if (ACAPI_TeamworkControl_HasConnection ()) {
			API_Guid objectGuid = ACAPI_TeamworkControl_FindLockableObjectSet ("GeoLocation");
			if (!ACAPI_TeamworkControl_HasDeleteModifyRight (objectGuid)) {
				ACAPI_WriteReport ("You have insufficient privileges to modify the GeoLocation settings.", true);
			} else {
				API_LockableStatus lockableStatus = ACAPI_TeamworkControl_GetLockableStatus (objectGuid);
...
static void	Do_ChangeHideShowZonesLegacyPreference (void)
{
	...
	err = ACAPI_Environment (ApiEnvSetPreferencesID, &legacyPrefs, (void *) APIPrefs_LegacyID);
	if (err != NoError)
		WriteReport_Alert ("Error");
}
...
static void Do_SetFilterAndCutSettings ()
{
	...
	err = ACAPI_Environment (APIEnv_Change3DImageSetsID, &filterAndCutSettings);
	if (err != NoError)
		WriteReport_Alert ("Error");
