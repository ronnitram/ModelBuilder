									...
									bool		ignoreFirstCoord)
{
	...
	ACAPI_Database (APIDb_GetCurrentDatabaseID, &currentDB, nullptr);
	if (isModel) {
	...
	if (isModel)
		ACAPI_Database (APIDb_ChangeCurrentDatabaseID, &currentDB, nullptr);
...
static void		Do_Switch3DCutaway ()
{
	...
	GSErrCode err = ACAPI_Interface (APIIo_Is3DCutawayEnabledID, &status, nullptr);
	if (err != NoError) {
		ACAPI_WriteReport ("APIIo_Is3DCutawayEnabledID failed!", true);
	} else {
		status = !status;
		err = ACAPI_Interface (APIIo_Set3DCutawayStatusID, &status, nullptr);
		if (err != NoError)
...
static void		Do_SwitchCuttingPlanes ()
{
	...
	GSErrCode err = ACAPI_Interface (APIIo_IsCuttingPlanesVisibleID, &status, nullptr);
	if (err != NoError) {
		ACAPI_WriteReport ("APIIo_IsCuttingPlanesVisibleID failed!", true);
	} else {
		status = !status;
		err = ACAPI_Interface (APIIo_SetCuttingPlanesVisibilityID, &status, nullptr);
		if (err != NoError)
			ACAPI_WriteReport ("APIIo_SetCuttingPlanesVisibilityID failed!", true);
	}
...
{
	ACAPI_Interface_InvokeForwardDialog ();
...
static void		Do_ChooseLibraryPart (void)
{
	...
	ACAPI_Interface (APIIo_OpenLibPartFileDialogID, &wasAnyLibPartChosen, &chosenLibPartLocation, &isGSM, &wasExternalEditorUsed);
...
static void		Do_ChoosePicture (void)
{
	...
	ACAPI_Interface (APIIo_OpenPictureDialogID, &wasAnyPictureChosen, &chosenPictureLocation);
...
static void		Do_ChooseZoneStamp (void)
{
	...
	ACAPI_Interface (APIIo_OpenOtherObjectDialogID, &wasAnyZoneStampChosen, &chosenZoneStampLocation, &isZoneStamp);
