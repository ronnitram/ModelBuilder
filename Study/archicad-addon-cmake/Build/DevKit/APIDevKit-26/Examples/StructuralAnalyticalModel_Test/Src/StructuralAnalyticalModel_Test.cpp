...
#include "APIEnvir.h"
#include "ACAPinc.h"
#include "ACAPI_Analytical_Adapter.hpp"
...
static GS::Array<API_AnalyticalCurveMember> GetCurveMembers () 
{
	GS::Array<API_AnalyticalCurveMember> api_curveMembers = {
		{																			// Column
			...
			{ API_CrossSection_Rectangle, 0.3, 0.4, 0.0, 0, { 1 } },				// crossSection
			{																		// curve
		...
		{																			// Beam
			...
			{ API_CrossSection_Circle, 0.5, 0.5, 0.0, 0, { 2 } },					// crossSection
			{																		// curve
				API_AnalyticalCurve_LineType,										// type
...
static GSErrCode  CreateLoadGroupsAndCasesAndCombinations ()
{
	...
	GSErrCode err = ACAPI_AnalyticalModel_CreateAnalyticalLoadGroup (loadGroup1);
	if (err != NoError)
		return err;

	API_AnalyticalLoadGroup loadGroup2 = GetAnalyticalLoadGroup ("LG2");

	err = ACAPI_AnalyticalModel_CreateAnalyticalLoadGroup (loadGroup2);
	if (err != NoError)
		return err;
	...
	err = ACAPI_AnalyticalModel_CreateAnalyticalLoadCase (loadCase1);
	if (err != NoError)
		return err;

	err = ACAPI_AnalyticalModel_CreateAnalyticalLoadCase (loadCase2);
	if (err != NoError)
		return err;

	err = ACAPI_AnalyticalModel_CreateAnalyticalLoadCase (loadCase3);
	if (err != NoError)
		return err;

	API_AnalyticalLoadCase loadCase4 = GetAnalyticalLoadCase ("LC4", loadGroup2.guid);
	err = ACAPI_AnalyticalModel_CreateAnalyticalLoadCase (loadCase4);
	if (err != NoError)
		return err;
	...
	err = ACAPI_AnalyticalModel_CreateAnalyticalLoadCombination (loadCombination1);
	if (err != NoError)
		return err;
	...
	API_AnalyticalLoadCombination loadCombination2 = GetAnalyticalLoadCombination ("COM2", loadCase1.loadCaseGuid, loadCase4.loadCaseGuid,
																					...
																					GS::NoValue, 0.2, 2.1, 0.4, 0.7);

	err = ACAPI_AnalyticalModel_CreateAnalyticalLoadCombination (loadCombination2);
	if (err != NoError)
		return err;
	...
	err = ACAPI_AnalyticalModel_AddLoadCaseToLoadCombination (loadFactor1, loadCombination1.guid);
	if (err != NoError)
		return err;

	err = ACAPI_AnalyticalModel_AddLoadCaseToLoadCombination (loadFactor2, loadCombination2.guid);
	if (err != NoError)
		return err;
...
static GSErrCode CreateCurveMembers () 
{
	...
	for (API_AnalyticalCurveMember& item : curveMembers) {
		err = ACAPI_AnalyticalModel_CreateElementFromCurveMember (item, GS::EmptyUniString);
		if (err != NoError)
			return err;
	}
	...
	for (const auto& curveMember : curveMembers) {
		...
		err = ACAPI_AnalyticalModel_SetAnalyticalRelease (curveMember.elemGuid, releaseType);
		if (err != NoError)
			return err;
	}
...
static GSErrCode CreateSurfaceMembers (API_ImportIdMappingType&	idMapping)
{
	...
	for (API_AnalyticalSurfaceMember& surfaceMember : surfaceMembers) {
		err = ACAPI_AnalyticalModel_CreateElementFromSurfaceMember (surfaceMember, "", &idMapping);
		if (err != NoError)
			return err;
	}
	...
	for (const auto& edgeMappingEntry : idMapping.edges) {
		...
		err = ACAPI_AnalyticalModel_SetAnalyticalRelease (edgeMappingEntry.key->memberGuid, releaseType);
		if (err != NoError)
			return err;
	}
...
static GSErrCode CreateAnalyticalLinks (const API_ImportIdMappingType& idMapping)
{
	...
	ACAPI_AnalyticalModel_SetElementFromLink (analytical0DLink, link0DElement);
	link0DElement.header.type = API_AnalyticalLinkID;
	link0DElement.header.layer = 1;
	...
	ACAPI_AnalyticalModel_SetElementFromLink (analytical1DLink, link1DElement);
	link1DElement.header.type = API_AnalyticalLinkID;
	link1DElement.header.layer = 1;
	...
	ACAPI_AnalyticalModel_SetElementFromLink (analytical2DLink, link2DElement);
	link2DElement.header.type = API_AnalyticalLinkID;
	link2DElement.header.layer = 1;
...
static GSErrCode CreateAnalyticalSupports (const API_ImportIdMappingType& idMapping)
{
	...
	ACAPI_AnalyticalModel_SetElementFromSupport (analyticalSupportType, supportElement);
	supportElement.header.type = API_AnalyticalSupportID;
	supportElement.header.layer = 1;
...
static GSErrCode CreateAnalyticalLoads ()
{
	...
	err = ACAPI_AnalyticalModel_GetAllAnalyticalLoadCases (loadCases);
	...
	ACAPI_AnalyticalModel_SetElementFromPointLoad (pointLoadType, pointLoadElement);
	pointLoadElement.header.type = API_AnalyticalPointLoadID;
	pointLoadElement.header.layer = 1;
	...
	ACAPI_AnalyticalModel_SetElementFromEdgeLoad (edgeLoadType, firstEdgeLoadElement);
	firstEdgeLoadElement.header.type = API_AnalyticalEdgeLoadID;
	firstEdgeLoadElement.header.layer = 1;
	...
	ACAPI_AnalyticalModel_SetElementFromEdgeLoad (beamEdgeLoadType, secondEdgeLoadElement);

	secondEdgeLoadElement.header.type = API_AnalyticalEdgeLoadID;
	secondEdgeLoadElement.header.layer = 1;
	...
	ACAPI_AnalyticalModel_GetEdgeLoadFromElement (firstEdgeLoadElement, firstEdgeLoad);
	...
	ACAPI_AnalyticalModel_SetElementFromEdgeLoad (firstEdgeLoad, firstEdgeLoadElement);
	...
	for (const API_AnalyticalCurve& curve : curves) {
		...
		err = ACAPI_AnalyticalModel_SetMemoFromEdgeLoadCustomData (edgeLoadCustomData, edgeLoadMemo);
	...
	ACAPI_AnalyticalModel_SetElementFromSurfaceLoad (surfaceLoadType, surfaceLoadElement);

	surfaceLoadElement.header.type = API_AnalyticalSurfaceLoadID;
	surfaceLoadElement.header.layer = 1;
	...
	ACAPI_AnalyticalModel_SetElementFromSurfaceLoad (customSurfaceLoadType, customSurfaceLoadElement);

	customSurfaceLoadElement.header.type = API_AnalyticalSurfaceLoadID;
	customSurfaceLoadElement.header.layer = 1;
	...
	ACAPI_AnalyticalModel_SetMemoFromSurfaceLoadCustomData (surfaceLoadCustomData, surfaceLoadMemo);
...
static GSErrCode SetAnalyticalModelVisibilitySettings ()
{
	...
	GSErrCode err = ACAPI_AnalyticalModel_GetVisibilitySettings (visibilitySettings);
	if (err != NoError)
		return err;
	...
	return ACAPI_AnalyticalModel_SetVisibilitySettings (visibilitySettings);
}
...
static void WriteLocalCoordinateSystemInIntersection (const API_AnalyticalLinkType& linkType)
{
	...
	ACAPI_AnalyticalModel_GetLCSOfAnalyticalMemberInIntersection (linkType.startElementPartRef, linkType.endElementPartRef, coordinateSystem);
...
static GSErrCode ListStructuralAnalyticalElements () 
{
	...
	API_Guid amGuid;
	GSErrCode err = ACAPI_AnalyticalModel_GetCurrentAnalyticalModel (amGuid);
	if (err != NoError)
		return err;
	...
	err = ACAPI_AnalyticalModel_GetCurveElements (amGuid, curveElements);
	if (err != NoError)
		return err;
	...
	for (auto& curveElement : curveElements) {
		...
		err = ACAPI_AnalyticalModel_GetCurveMember (amGuid, curveElement, api_curveMember);
		if (err != NoError)
			return err;
		...
		err = ACAPI_AnalyticalModel_GetAnalyticalRelease (amGuid, curveElement, api_releaseElement);
		if (err != NoError)
			return err;
	...
	WriteReportForElements (linkElements, [](const API_Element& api_element) {
		...
		ACAPI_AnalyticalModel_GetLinkFromElement (api_element, link);
		if (IsLinkInIntersection (link)) {
			WriteLocalCoordinateSystemInIntersection (link);
		}

		GS::UniString elemTypeName;
		ACAPI_Goodies_GetElemTypeName (api_element.header.type, elemTypeName);
	...
	WriteReportForElements (supportElements, [](const API_Element& api_element) {
		...
		ACAPI_AnalyticalModel_GetSupportFromElement (api_element, support);
	...
	err = ACAPI_AnalyticalModel_GetAnalyticalLoadGroups (api_loadGroups);
	...
	err = ACAPI_AnalyticalModel_GetAllAnalyticalLoadCases (api_loadCases);
	...
	err = ACAPI_AnalyticalModel_GetAllAnalyticalLoadCases (api_loadCombinations);
	...
	WriteReportForElements (pointLoadElements, [] (const API_Element& api_element) {
		...
		ACAPI_AnalyticalModel_GetPointLoadFromElement (api_element, load);
	...
	WriteReportForElements (edgeLoadElements, [](const API_Element& api_element) {
		...
		ACAPI_AnalyticalModel_GetEdgeLoadFromElement (api_element, load);
	...
	WriteReportForElements (surfaceLoadElements, [](const API_Element& api_element) {
		...
		ACAPI_AnalyticalModel_GetSurfaceLoadFromElement (api_element, load);
...
{
	GSErrCode err = ACAPI_Register_Menu (STRUCTURAL_ANALYTICAL_MODEL_TEST_MENU_STRINGS, STRUCTURAL_ANALYTICAL_MODEL_TEST_MENU_PROMPT_STRINGS, MenuCode_UserDef, MenuFlag_Default);
...
{
	GSErrCode err = ACAPI_Install_MenuHandler (STRUCTURAL_ANALYTICAL_MODEL_TEST_MENU_STRINGS, MenuCommandHandler);
