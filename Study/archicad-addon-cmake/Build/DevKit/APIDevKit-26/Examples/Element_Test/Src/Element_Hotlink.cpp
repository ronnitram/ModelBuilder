...
static GS::Optional<API_Element>	GetPlacedHotlinkByClickOnElement ()
{
	...
	GSErrCode err = ACAPI_Goodies (APIAny_GetContainingHotlinkGuidID, &guid, &hotlinkElem.header.guid);
...
GSErrCode	Do_UpdateHotlink ()
{
	...
	if (hotlinkElem.HasValue ()) {
		err = ACAPI_Database (APIDb_UpdateHotlinkCacheID, const_cast<API_Guid*> (&hotlinkElem->hotlink.hotlinkNodeGuid));
...
GSErrCode	Do_DeleteHotlink ()
{
	...
	if (hotlinkElem.HasValue ()) {
		err = ACAPI_CallUndoableCommand ("Element Test - Delete Hotlink Node", [&] () -> GSErrCode {
			return ACAPI_Database (APIDb_DeleteHotlinkNodeID, const_cast<API_Guid*> (&hotlinkElem->hotlink.hotlinkNodeGuid));
		});
...
GSErrCode	Do_BreakHotlink ()
{
	...
	if (hotlinkElem.HasValue ()) {
		err = ACAPI_CallUndoableCommand ("Element Test - Break Hotlink Node", [&] () -> GSErrCode {
			return ACAPI_Database (APIDb_BreakHotlinkNodeID, const_cast<API_Guid*> (&hotlinkElem->hotlink.hotlinkNodeGuid));
		});
...
static GS::UniString	GetLocationDetailsOfHotlinkNode (const API_HotlinkNode& hotlinkNode)
{
	switch (hotlinkNode.sourceType) {
		...
		case APIHotlink_TWFS: {
			...
			ACAPI_TeamworkControl_GetTeamworkProjectDetails (*hotlinkNode.sourceLocation,
															 &serverUrl,
															 &projectName,
															 &userName);
...
void	Do_ListHotlinkProxyElements ()
{
	...
	GSErrCode err = ACAPI_Database (APIDb_GetHotlinkProxyElementTableID, const_cast<API_Guid*> (&hotlinkElem->header.guid), &proxyElementTable);
