...
static void		Do_CMD1_Call (void)
{
	...
	mdid.developerID = MDID_APICD;
	mdid.localID = MDID_APICD_CommunicationClient;
	err = ACAPI_Command_Call (&mdid, CmdID_PutText, 1, parHdl, nullptr, avoidDialog);

	if (parHdl != nullptr) {
		err = ACAPI_Goodies (APIAny_FreeMDCLParameterListID, &parHdl);
		if (err != NoError)
...
static void		Do_LinkToClient (void)
{
	...
	err = ACAPI_Command_Call (&mdid, CmdID_BegBinaryLink, 1, nullptr, (GSPtr) &procTbl, false);
	if (err != NoError) {
	...
	err = ACAPI_Command_Call (&mdid, CmdID_EndBinaryLink, 1, nullptr, nullptr, false);
	if (err != NoError) {
		ErrorBeep ("CmdID_BegBinaryLink", err);
		return;
	}
...
static void		Do_Dump3DModel (void)
{
	...
	err = ACAPI_Command_Call (&mdid, 'DUMP', 1, nullptr, nullptr, false);
	if (err != NoError) {
		ErrorBeep ("\'DUMP\'", err);
		return;
	}
...
static void		Do_CallExternalSaveAsDWG (void)
{
	...
	IO::Location loc;
	if (err == NoError) {
		err = ACAPI_Command_Call (&mdid, 'GDCO', 1, parHdl, (GSPtr)&loc, false);	// get default translator
	}
	...
	while (waitingForProjectCounter > 0) {
		if (procValue != numOfTasks - waitingForProjectCounter) {
			...
			ACAPI_Interface (APIIo_SetProcessValueID, &procValue);
		}
