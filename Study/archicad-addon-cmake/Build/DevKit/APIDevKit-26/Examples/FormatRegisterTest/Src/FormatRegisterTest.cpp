...
GSErrCode	__ACDLL_CALL	RegisterInterface (void)
{
	...
	GSErrCode err = ACAPI_Register_PublishFormat (formatData);
...
GSErrCode		__ACENV_CALL	SaveProcImp (API_SaveMethod /*method*/, const char* path, API_UserData* userData, GS::UniString* reportStr)
{
...
GSErrCode		__ACENV_CALL	SaveOptionProcImp (API_SaveMethod /*method*/, API_UserData* userData)
{
	FormatTranslator locTranslator = GetFormatTranslator (userData->dataHdl);
...
GSErrCode		__ACENV_CALL	TranslatorGetterProcImp (GS::Array<API_TranslatorNameConfig>& arr) 
{
	arr.Clear ();
...
GSErrCode __ACENV_CALL DefaultTranslatorGetterProcImp (API_TranslatorNameConfig& def) 
{
...
{
	GSErrCode err = ACAPI_Install_PublisherSaveMethod ("text/Testxml", SaveProcImp);

	err |= ACAPI_Install_PublisherSaveOption ("text/Testxml", SaveOptionProcImp);
	err |= ACAPI_Install_PublisherTranslatorGetter ("text/Testxml", TranslatorGetterProcImp);
	err |= ACAPI_Install_PublisherDefaultTranslatorGetter ("text/Testxml", DefaultTranslatorGetterProcImp);
	DBASSERT (err == NoError);
