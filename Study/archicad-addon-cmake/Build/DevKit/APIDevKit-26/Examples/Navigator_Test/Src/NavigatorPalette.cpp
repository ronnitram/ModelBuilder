							 ...
							 API_DatabaseInfo* dbUnId = nullptr)
{
	...
	if (itemType == API_DocumentFrom3DNavItem && dbUnId != nullptr) {
		...
		ACAPI_Environment (APIEnv_GetDocumentFrom3DSettingsID, (void*)&dbUnId->databaseUnId, &d3dt);
...
static void MakeTree (short dialogID, short	treeItem, Int32 parent, const API_Guid& guid, GS::PagedArray<Int32>& itemsArray, API_NavigatorMapID mapId)
{
	...
	for (API_NavigatorItem& childItem : items) {
		if (childItem.itemType == API_ScheduleNavItem) {
			...
			err = ACAPI_Navigator (APINavigator_SearchNavigatorItemID, &childItem, nullptr, &items2);
...
static void RenamePublisherItem (short dialogID)
{
	...
	for (USize i = 0; i < toRenameItems.GetSize (); ++i) {
		...
		GSErrCode err = ACAPI_Navigator (APINavigator_GetNavigatorItemID, &toRenameItems[i], &navItem);
		if (err != NoError)
			continue;
		...
		ACAPI_Navigator (APINavigator_ChangeNavigatorItemID, &navItem, nullptr);
	}
...
static void DeleteView (short dialogID)
{
	...
	for (USize i = 0; i < toDeleteItems.GetSize (); ++i)
		ACAPI_Navigator (APINavigator_DeleteNavigatorViewID, &toDeleteItems[i], &silentMode);
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
inline bool Change_Layer (API_AttributeIndex	layerID, short status)
{
	API_Attribute	layer;
...
static void PublishTest ()
{
	...
	if (selected != DG_TVI_NONE) {
		...
		if (guid != GS::NULLGuid) {
			...
			GSErrCode err = ACAPI_Navigator (APINavigator_GetNavigatorItemID, &guid, &navItem);
			...
			if (err == NoError) {
				...
				err = ACAPI_Automate (APIDo_PublishID, &publishPars, nullptr);
															...
															DGMessageData		msgData)
{
		...
		if (item == NavTreeView) {
			...
			if (guid != GS::NULLGuid) {
				...
				if (err == NoError  && navItem.db.typeID == APIWind_3DModelID) {
					...
					BNZeroMemory (&projInfo, sizeof (projInfo));
					err = ACAPI_Environment (APIEnv_Get3DProjectionSetsID, &projInfo);
					...
					if (navItem.itemType == API_AxonometryNavItem && projInfo.isPersp) {
						...
						ACAPI_Environment (APIEnv_Change3DProjectionSetsID, &projInfo, &switchOnlyAxoOrPersp);
					}
				...
				if (err == NoError) {
					err = ACAPI_Automate (APIDo_ChangeWindowID, &navItem.db, nullptr);
				}
				...
				if (err == NoError && navItem.db.typeID == APIWind_FloorPlanID) {
					...
					err = ACAPI_Environment (APIEnv_ChangeStorySettingsID, &cmd, nullptr);
				}
				...
				if (err == NoError) {
					...
					BNZeroMemory (&navView, sizeof (navView));
					BNZeroMemory (&navItem, sizeof (navItem));
					navItem.guid = guid;
					navItem.mapId = NavigatorPaletteTestGlobals::GetInstance ().GetNavigatorMapID ();

					GSErrCode	err = ACAPI_Navigator (APINavigator_GetNavigatorViewID, &navItem, &navView);
					if (err == NoError) {
						...
						if (navView.saveLaySet) {
							if (navView.layerCombination[0] != 0) {
								...
								Int32 nLayer = BMhGetSize ((GSHandle)navView.layerStats) / Sizeof32 (API_LayerStat);
								for (Int32 layer = 0; layer < nLayer; layer++) {
									needRebuild = needRebuild || Change_Layer ((*navView.layerStats)[layer].lInd,
										(*navView.layerStats)[layer].lFlags);
								}
							}
						}
						if (navView.saveDispOpt) {
							...
							err = ACAPI_Environment (APIEnv_ChangeViewOptionsID, &viewOptions, nullptr);
						}
						...
						if (err == NoError && navView.saveDScale) {
							...
							err = ACAPI_Database (APIDb_ChangeDrawingScaleID, &scale, (void*)((GS::IntPtr)(!navView.saveZoom)));
						}
						if (err == NoError && navView.saveZoom)
							err = ACAPI_Automate (APIDo_ZoomID, &navView.zoom, nullptr);
						...
						if (needRebuild) {
							bool regener = true;
							err = ACAPI_Automate (APIDo_RebuildID, &regener, nullptr);
						}
					}
				}
			}
		...
		case NavTreeView:
		{
			...
			if (treeChange->actionFlag == DG_TVCF_SELECTION) {
				...
				if (guid != GS::NULLGuid) {
					...
					if (isPublisher) {
						err = ACAPI_Navigator (APINavigator_GetNavigatorItemID, &guid, &navItem);
						if (err == NoError)
					...
					err = ACAPI_Navigator (APINavigator_GetNavigatorViewID, &navItem, &navView);
					if (err == NoError) {
						if (navView.modelViewOpt != nullptr) {
							ACAPI_FreeGDLModelViewOptionsPtr (&navView.modelViewOpt->gdlOptions);
							BMKillPtr ((GSPtr*)&navView.modelViewOpt);
	...
	case DG_MSG_DRAGDROP:
	{
		...
		if (ddData->subMessage == DG_DRAG_STARTING) {
			...
			if (ACAPI_Navigator (APINavigator_SetNavigatorItemPositionID, &sourceGuid, &parentGuid, &childGuid) == NoError) {
				UpdateTree (dialogID);
			}
		}
	}
