...
{
	GSErrCode err = ACAPI_Database (APIDb_StartDrawingDataID, &scale);
	if (err == NoError) {
		NavigatorUtility::CopyFloorplanElements (viewPointID);
		NavigatorUtility::GetFloorplanElements (viewPointID, elems);
	}
	err = ACAPI_Database (APIDb_StopDrawingDataID, &idfStore, &boundingBox);
...
GSErrCode	NavigatorCallbackInterface::NewItem (const API_Guid& viewPointID) const
{
	...
	GSErrCode err = ACAPI_Navigator (APINavigator_GetNavigatorVPItemID, &vpData);
	if (err == NoError) {
		...
		if (newItemDialog.Invoke ()) {
			...
			return ACAPI_Navigator (APINavigator_CreateNavigatorVPItemID, &data);
		}
	}
...
GSErrCode	NavigatorCallbackInterface::DeleteItem (const GS::Array<API_Guid>& viewPointIDList) const
{
	for (const API_Guid& guid : viewPointIDList) {
		...
		GSErrCode err = ACAPI_Navigator (APINavigator_GetNavigatorVPItemID, &vp);
		if (err != NoError)
			return err;

		if (vp.itemType != API_NavigatorAddOnViewPointRootID) {
			err = ACAPI_Navigator (APINavigator_DeleteNavigatorVPItemID, &vp.guid);
			if (err != NoError)
				return err;
		}
	}
...
GSErrCode	NavigatorCallbackInterface::RenameItem (const API_Guid& viewPointID) const
{
	...
	if (err == NoError && vpData.itemType != API_NavigatorAddOnViewPointRootID) {
		...
		if (renameItemDialog.Invoke ()) {
			...
			ACAPI_Navigator (APINavigator_ChangeNavigatorVPItemID, &vpData);
		}
	}
...
GSErrCode NavigatorCallbackInterface::ExecuteMergePostProcess () const
{
	...
	err = ACAPI_Navigator (APINavigator_GetNavigatorVPItemChildrenID , const_cast<API_Guid*> (&RootGuid), &children);
	if (err != NoError)
		return err;
