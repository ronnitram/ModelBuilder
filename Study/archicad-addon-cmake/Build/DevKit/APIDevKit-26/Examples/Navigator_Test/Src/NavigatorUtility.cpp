...
GSErrCode		NavigatorUtility::GetFloorplanElements (const API_Guid& guid, GS::Array<API_Guid>& elems)
{
	...
	GSErrCode err = ACAPI_Navigator (APINavigator_GetNavigatorVPItemID, &navigatorAddOnViewPointData);
	if (err != NoError)
		return err;
...
void NavigatorUtility::ResolveNameConflict (GS::HashTable<GS::UniString, API_NavigatorAddOnViewPointData> itemNames, API_NavigatorAddOnViewPointData conflictingVpData)
{
	...
	DBVERIFY (ACAPI_Navigator (APINavigator_ChangeNavigatorVPItemID, &conflictingVpData) == NoError);
}
...
bool NavigatorUtility::IsTeamworkProject (short* currentUserId)
{
	...
	GSErrCode err = ACAPI_Environment (APIEnv_ProjectID, &pi);
...
bool NavigatorUtility::HasDeleteModifyRight ()
{
	...
	bool hasRight = false;
	ACAPI_Environment (APIEnv_GetTWAccessRightID, reinterpret_cast<void*> (APISchedulesAndIndexesDeleteModify), &hasRight);		// nincs kulon API-s jog, az IES feltetelei a megfeleloek by design
	return hasRight;
}


GSErrCode	__ACENV_CALL	NavigatorUtility::APINavigatorAddOnViewPointDataMergeHandler (const GS::Array<API_NavigatorAddOnViewPointData>&	sourceVPDataArray)
{
	...
	for (const API_NavigatorAddOnViewPointData& vp : sourceVPDataArray) {
		if (vp.itemType == API_NavigatorAddOnViewPointRootID) {
			...
			DBVERIFY (ACAPI_Navigator (APINavigator_CreateNavigatorVPItemID, &createdVP) == NoError);
...
GSErrCode	__ACENV_CALL	NavigatorUtility::APINavigatorAddOnViewPointDataSaveOldFormatHandler (API_FTypeID												/*planFileType*/,
...
GSErrCode	__ACENV_CALL	NavigatorUtility::APINavigatorAddOnViewPointDataConvertNewFormatHandler (API_FTypeID											/*planFileType*/,
...
GSErrCode	NavigatorUtility::RegisterNavigatorRootGroup ()
{
	...
	err = ACAPI_Navigator (APINavigator_GetNavigatorVPRootGroupsID, &roots);
	if (!roots.IsEmpty ())
		return err;
	...
	err = ACAPI_Navigator (APINavigator_CreateNavigatorVPItemID, &navigatorAddOnViewPointData);
	if (err != NoError)
...
API_Guid	GetDeepestChildFolder (const API_Guid& parentId)
{
	...
	GSErrCode err = ACAPI_Navigator (APINavigator_GetNavigatorVPItemChildrenID, &guid, &children);
	if (err == NoError) {
		for (auto it = children.Enumerate (); it != nullptr; ++it) {
			API_NavigatorAddOnViewPointData vpData;
			vpData.guid = *it;
			err = ACAPI_Navigator (APINavigator_GetNavigatorVPItemID, &vpData);
			if (err == NoError) {
...
GSErrCode	NavigatorUtility::CreateNavigatorChildGroup ()
{
	...
	GSErrCode err = ACAPI_Navigator (APINavigator_CreateNavigatorVPItemID, &navigatorAddOnViewPointData);
	if (err != NoError) {
		ACAPI_WriteReport ("APINavigator_CreateNavigatorVPItemID returned error", false);
	}
...
GSErrCode NavigatorUtility::CloneProjectMapToViewMap (void)
{
	...
	API_Guid clonedGuid = {};
	err = ACAPI_Navigator (APINavigator_CloneProjectMapItemToViewMapID, &sourceGuid, &parentGuid, &clonedGuid);
	if (err != NoError) {
		return err;
	}
