...
static GSErrCode GetValidatorInfo (const API_Guid& userRefId, API_WindowValidatorInfo& validatorInfo)
{
	...
	GSErrCode err = ACAPI_Navigator (APINavigator_GetNavigatorVPItemID, &viewPointData);
...
static void UpdateContentValidator (const API_Guid& userRefId)
{
	...
	DBVERIFY (ACAPI_Database (APIDb_RebuildWindowValidatorID, &validatorInfo) == NoError);
}


static void UpdateContentValidator (API_WindowValidatorInfo& validatorInfo)
{
	DBVERIFY (ACAPI_Database (APIDb_RebuildWindowValidatorID, const_cast<API_WindowValidatorInfo*> (&validatorInfo)) == NoError);
}
...
static GSErrCode		WindowHandlerCallBack (const API_Guid& userRefId, API_NotifyWindowEventID notifID)
{
	...
	case APINotifyWindow_Activate: {
		...
		DBVERIFY (GetValidatorInfo (userRefId, validatorInfo) == NoError);
		DBVERIFY (ACAPI_Database (APIDb_CheckWindowValidatorID, &validatorInfo, &contentChanged) == NoError);
	...
	case APINotifyWindow_Close:
		DBVERIFY (ACAPI_Database (APIDb_DestroyWindowValidatorID, const_cast<API_Guid*> (&userRefId)) == NoError);
		break;
	}
...
static GS::Array<AddOnNavigatorViewPointData>	GetRootVPData ()
{
	...
	if (ACAPI_Navigator (APINavigator_GetNavigatorVPItemChildrenID, &rootGuid, &guids) == NoError) {
		for (auto it = guids.Enumerate (); it != nullptr; ++it) {
			API_NavigatorAddOnViewPointData data;
			data.guid = *it;
			if (ACAPI_Navigator (APINavigator_GetNavigatorVPItemID, &data) == NoError && data.itemType == API_NavigatorAddOnViewPointNodeID) {
...
NavigatorWindowHandling::APIWindowGuard::~APIWindowGuard ()
{
	GSErrCode err = ACAPI_Database (APIDb_SetZoomID);
	if (err == NoError)
...
GSErrCode	NavigatorWindowHandling::OpenWindow (const API_Guid& guid, bool newWindow)
{
	...
	if (wdIdsContainsGuid || (!newWindow && !wdIds.IsEmpty ())) {
		...
		if (!wdIdsContainsGuid) {
			DBVERIFY (ACAPI_Database (APIDb_SetWindowIdID, &wdIds[0], const_cast<API_Guid*> (&guid)) == NoError);
		}

		err = ACAPI_Automate (APIDo_ChangeWindowID, &windowInfo);
	} else {
		...
		err = ACAPI_Database (APIDb_NewWindowID, &windowPars, (void*)WindowHandlerCallBack);
		API_WindowValidatorInfo validatorInfo;
		DBVERIFY (GetValidatorInfo (guid, validatorInfo) == NoError);
		DBVERIFY (ACAPI_Database (APIDb_BuildWindowValidatorID, &validatorInfo) == NoError);
	}
...
void	NavigatorWindowHandling::RegenerateContentIfAppropriate ()
{
	const GS::Optional<API_Guid> refId = [&] {
		...
		if (ACAPI_Database (APIDb_GetCurrentWindowID, &winInfo, nullptr) != NoError || winInfo.typeID != APIWind_MyDrawID) {
