				...
				case DG_MSG_INIT:
					{
						...
						ACAPI_Interface_CreateAttributePicker (app, prefsData->attributePicker);
...
static GSErrCode	Do_FillSettings (void)
{
	...
	if (version == CURR_ADDON_VERS) {
		...
		if (platformSign != GS::Act_Platform_Sign) {
			...
			IVLong (inplatform, &prefsData.layer);
			IVLong (inplatform, &prefsData.hatchLineType);
			IVLong (inplatform, &prefsData.hatchFill);
...
static void SetListView (short dialId, short lvItem, GSHandle ***gsh)
{
	...
	*gsh = (GSHandle **) BMhAllClear (nLibParts * sizeof (GSHandle));
	...
	ACAPI_Interface (APIIo_InitProcessWindowID, &phaseText, &nLibParts, &pcType);

	BNZeroMemory (&lp, sizeof (API_LibPart));
	// for all library parts
	for (lp.index = 1, nsh = 0; lp.index <= nLibParts; lp.index++) {
		ACAPI_Interface (APIIo_SetNextProcessPhaseID, &phaseText, &one);
		if (ACAPI_LibPart_Get (&lp) == NoError) {
			...
			if (lp.typeID == APILib_ObjectID && !lp.isTemplate && lp.isPlaceable) {
				...
				err = ACAPI_LibPart_GetSection (lp.index, &lpSect, &mimePic.inputHdl, nullptr);
				if (err == NoError) {
					...
					err = ACAPI_Goodies (APIAny_ConvertMimePictureID, (GSPtr) &mimePic, nullptr);
				}
		...
		if (ACAPI_Interface (APIIo_IsProcessCanceledID) == APIERR_CANCEL)
			break;
	}
...
{
	API_Attribute		attr;
	API_AttributeIndex	ii, nMaterials, nsh;
	short				one = 1;
	...
	*gsh = (GSHandle **) BMhAllClear (nMaterials * sizeof (GSHandle));
	...
	for (ii = 1, nsh = 0; ii <= nMaterials; ++ii) {
		ACAPI_Interface (APIIo_SetProcessValueID, &one, nullptr);
		...
		if (ACAPI_Interface (APIIo_IsProcessCanceledID, nullptr, nullptr) == APIERR_CANCEL)
			break;
	}
...
static void		OnlyOneAttr (API_AttributeIndex index, API_AttributeIndex* n, API_AttributeIndex*** indHdl)
{
	if (index > 0) {
		*n = 1;
		*indHdl = (API_AttributeIndex**) BMhAll ((Int32) sizeof (API_AttributeIndex));
		if (*indHdl != nullptr)
			(**indHdl)[0] = index;
	} else {
		*n = 0;
...
GSErrCode		AllNeededAttrs (API_AttributeIndex* attributeNumber, API_AttributeIndex*** indexHandle, Int32 custIndex)
{
	DBASSERT (indexHandle != nullptr);
	if (indexHandle == nullptr)
		return APIERR_BADPARS;

	API_AttributeIndex		maxIndex = 0;
	...
	*indexHandle = reinterpret_cast<API_AttributeIndex**> (BMhAllClear ((custIndex + 1) * sizeof (API_AttributeIndex)));
	...
	for (API_AttributeIndex attributeIndex = 1; attributeIndex <= custIndex; attributeIndex++) {	//first 10 attributes
	...
	if (attributeNumber != nullptr)
		*attributeNumber = index;

	Int32	newLength = (index + 1) * sizeof (API_AttributeIndex);
...
Int32	GetMaterListSize (UC257CallBackRec* callBackRec, bool general)
{
	...
	API_AttributeIndex		nCells;
	API_AttributeIndex**	indHdl = nullptr;
...
GSErrCode	GetMaterInfo (API_AttributeIndex index, UC257CellData* cell)
{
...
static void*	GetMaterList (UC257CallBackRec* callBackRec, bool general)
{
	...
	short					i;
	API_AttributeIndex		nAttr, nCells;
	API_AttributeIndex**	indHdl = nullptr;
	...
	switch (message) {
		case DG_MSG_INIT:
		{
...

			{
				...
				DBVERIFY (ACAPI_Interface_CreateAttributePicker (app, wallProfilePicker) == NoError);
			}

			{
				...
				DBVERIFY (ACAPI_Interface_CreateAttributePicker (app, beamProfilePicker) == NoError);
			}

			{
				...
				DBVERIFY (ACAPI_Interface_CreateAttributePicker (app, columnProfilePicker) == NoError);
			}

			{
				...
				DBVERIFY (ACAPI_Interface_CreateAttributePicker (app, handrailProfilePicker) == NoError);
			}

			{
				...
				DBVERIFY (ACAPI_Interface_CreateAttributePicker (app, otherGDLObjectProfilePicker) == NoError);
			}

			{
				...
				DBVERIFY (ACAPI_Interface_CreateAttributePicker (app, allProfilePicker) == NoError);
			}
		} break;
...
static void		ToggleInstantDialogResourceLoadingStatus (void)
{
	...
	GSFlags itemFlags = 0;
	GSErrCode err = ACAPI_Interface (APIIo_GetMenuItemFlagsID, &itemRef, &itemFlags);
	if (err == NoError) {
		if (isEnabled) {
...
GSErrCode	__ACENV_CALL	RegisterInterface (void)
{
	...
	err = ACAPI_Register_Menu (DGTEST_MENU_STRINGSID, DGTEST_PROMPT_STRINGSID, MenuCode_UserDef, MenuFlag_Default);
	if (err != NoError)
