	...
	okButton			(GetReference (), OkButtonId)
{
	...
	const API_AttributeIndex maxBuildingMat = 50;
	API_AttributeIndex nBuildingMat = maxBuildingMat;
	...
	for (API_AttributeIndex i = 1; i <= nBuildingMat; i++) {
		buildMatList.InsertItem (static_cast<short>(i));	//conversion is not problem, because max value is 50.
		...
		buildMatList.SetTabItemText (static_cast<short>(i), BuildingMatNameTab, attr.header.name);
		buildMatList.SetTabItemText (static_cast<short>(i), PenIndexTab, GS::UniString::Printf ("%d", attr.buildingMaterial.cutFillPen));
	}
