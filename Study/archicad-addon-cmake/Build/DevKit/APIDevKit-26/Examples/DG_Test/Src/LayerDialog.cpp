	...
	statusTable 		(statusTableIn)
{
	...
	API_AttributeIndex nLayers = 0;
	ACAPI_Attribute_GetNum (API_LayerID, &nLayers);
	for (API_AttributeIndex i = 2; i <= nLayers; ++i) {
		ListBoxRow	row;
		API_Attribute attr;
...
void LayerDialog::SortListBy (const TabId sortBy)
{
	class ElementSorter {
	private:
		const LayerDialog&		dialog;
		TabId					sortBy;
		DG::ListBox::ArrowType	sortDirection;
	public:
		ElementSorter (const LayerDialog& dialogIn, GS::Array<ListBoxRow>& rows, TabId sortBy, DG::ListBox::ArrowType lastArrow) :
			dialog (dialogIn),
			sortBy (sortBy)
		{
		...
		bool operator () (const ListBoxRow& row1, const ListBoxRow& row2) {
			...
			result = name1.Compare (name2, GS::UniString::CaseInsensitive);
	...
	ElementSorter sorter (*this, sortedList, sortBy, layerList.GetHeaderItemArrowType ((short) sortBy));
