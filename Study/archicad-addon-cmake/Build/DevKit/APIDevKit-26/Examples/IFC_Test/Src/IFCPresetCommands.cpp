...
void CreatePreset ()
{
	...
	for (const auto& presetToCreate : presetsToCreate) {
		...
		GSErrCode err = ACAPI_Command_Call (&IfcMdid, CREATEPRESET, 1L, params.GetParams (), nullptr, false);
		if (err == NoError) {
...
void HasPreset ()
{
	...
	GSErrCode err = ACAPI_Command_Call (&IfcMdid, HASPRESET, 1L, params.GetParams (), reinterpret_cast<GSPtr> (&presetFound), false);
...
void ClearPreset ()
{
	...
	GSErrCode err = ACAPI_Command_Call (&IfcMdid, CLEARPRESET, 1L, params.GetParams (), nullptr, false);
	if (err == NoError) {
...
void AddNewTypeMappingForImport ()
{
	...
	GSErrCode err = ACAPI_Command_Call (&IfcMdid, ADDNEWTYPEMAPPINGFORIMPORT, 1L, params.GetParams (), nullptr, false);
	if (err == NoError) {
...
void AddNewTypeMappingForExport (const GS::UniString& ifcSchema /* = "" */)
{
	...
	GSErrCode err = ACAPI_Command_Call (&IfcMdid, ADDNEWTYPEMAPPINGFOREXPORT, 1L, params.GetParams (), nullptr, false);
	if (err == NoError) {
...
void AddNewPropertyMappingForExport ()
{
	...
	err = ACAPI_Command_Call (&IfcMdid, ADDNEWPROPERTYMAPPINGFOREXPORT, 1L, params.GetParams (), nullptr, false);
	if (err == NoError) {
		DG::InformationAlert (presetName + " export property mapping preset was successfully modified!", GS::UniString (), "OK");
	} else {
		DG::WarningAlert (presetName + " export property mapping preset could not be modified (err : " + GS::ValueToUniString (err) + ")!", GS::UniString (), "OK");
...
void AddNewPropertyMappingForImport ()
{
	...
	GSErrCode err = ACAPI_Command_Call (&IfcMdid, ADDNEWPROPERTYMAPPINGFORIMPORT, 1L, params.GetParams (), nullptr, false);
	if (err == NoError) {
