...
void		WRL::MAExporter::CreateTextureFile (const ModelerAPI::AttributeIndex& textureIndex, char* fullPathName)
{
	...
	if (name[0] == '\0')
		GS::csnprintf (fileName, sizeof (fileName), "%s%ld.jpg", localizedStrings[WRL::TextureName].ToCStr ().Get (), (GS::LongForStdio) textureIndex.GetIndex ());
	else
...
void		WRL::MAExporter::ExportCameraFromSaveAs3D  (void)
{
	...
	ACAPI_Environment (APIEnv_Get3DProjectionSetsID, &projSets, nullptr);
...
void		WRL::MAExporter::ExportBodies (void)
{
	...
	ACAPI_Interface (APIIo_SetNextProcessPhaseID, nullptr, &nElements);
	...
	try {
		for (Int32 iElement = 1; iElement <= nElements; iElement++) {
			...
			ACAPI_Interface (APIIo_SetProcessValueID, &iElement, nullptr);
			if (iElement % 1000 == 0) {
