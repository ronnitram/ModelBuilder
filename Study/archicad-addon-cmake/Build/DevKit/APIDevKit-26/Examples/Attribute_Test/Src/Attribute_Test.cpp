...
static void		EnumerateAttributes (API_AttrTypeID										typeID,
									 const std::function<bool (const API_Attribute&)>&	processor)
{
	API_AttributeIndex count;
	DBVERIFY (ACAPI_Attribute_GetNum (typeID, &count) == NoError);
...
static GS::Array<GS::Guid>	GetSomeAttributes (API_AttrTypeID typeID, USize maxAmount)
{
	...
	EnumerateAttributes (typeID, [&] (const API_Attribute& attribute) {
		guids.Push (APIGuid2GSGuid (attribute.header.guid));
		return guids.GetSize () < maxAmount;
	});
...
static const char* AttributeName (API_AttrTypeID inType, API_AttributeIndex inIndex)
{
	API_Attribute	attr;
		...
		switch (profileDescriptionIt->item_Typ) {
			...
			case SyHatch: {
					...
					Int32 uiPriority = 0;
					{
						...
						buildMat.head.index = (API_AttributeIndex) syHatch.GetBuildMatIdx ();
...
static void		Do_CreateLayerComb (void)
{
	...
	if (defs.layer_statItems == nullptr) {
...
static void		Do_ListLayerCombinations (void)
{
	API_Attribute		attrib;
	API_AttributeDef	defs;
	API_AttributeIndex	count, count2, i, j;
	GSErrCode				err;
...
static void Do_ImportFormatRegistration (void)
{
	...
	GSErrCode err = ACAPI_Register_AttributeImportFormat (formatData);

	if (err != NoError)
		DBPrintf ("Attribute Test:: RegisterInterface() ACAPI_Register_AttributeImportFormat failed\n");
}
...
GSErrCode	__ACDLL_CALL	RegisterInterface (void)
{
	...
	if (err == NoError)
		err = ACAPI_Register_Menu (32504, 0, MenuCode_UserDef, MenuFlag_SeparatorAfter);
...
GSErrCode __ACENV_CALL	Initialize (void)
{
	...
	if (enableImportFormatRegistration) {
		err = ACAPI_Install_AttrManagerImportMethod ("txt", ImportAttributeProcess);
		if (err != NoError)
			DBPrintf ("Attribute Test:: Initialize() ACAPI_Install_AttrManagerImportMethod failed\n");
	}
