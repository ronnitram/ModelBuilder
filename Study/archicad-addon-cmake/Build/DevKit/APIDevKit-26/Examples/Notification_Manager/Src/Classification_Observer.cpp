...
static GSErrCode RegisterClassificationSystemHandler ()
{
	...
	const auto result = ACAPI_Notify_RegisterEventHandler (GS::NewOwned<ClassificationSystemEventHandler> (), *classificationSystemEventHandlerId);
	if (result != NoError) {
...
static GSErrCode RegisterClassificationItemHandler ()
{
	...
	const auto result = ACAPI_Notify_RegisterEventHandler (GS::NewOwned<ClassificationItemEventHandler> (), *classificationItemEventHandlerId);
	if (result != NoError) {
...
GSErrCode	__ACENV_CALL RegisterClassificationEventHandlers (void)
{
...
GSErrCode	__ACENV_CALL UnRegisterClassificationEventHandlers (void)
{
	if (classificationItemEventHandlerId.HasValue ()) {
		ACAPI_Notify_UnregisterEventHandler (*classificationItemEventHandlerId);
		classificationItemEventHandlerId.Clear ();
	}

	if (classificationSystemEventHandlerId.HasValue ()) {
		ACAPI_Notify_UnregisterEventHandler (*classificationSystemEventHandlerId);
		classificationSystemEventHandlerId.Clear ();
	}
