...
static GSErrCode RegisterPropertyGroupHandler ()
{
	...
	const auto result = ACAPI_Notify_RegisterEventHandler (GS::NewOwned<PropertyGroupEventHandler> (), *PropertyGroupEventHandlerId);
	if (result != NoError) {
...
static GSErrCode RegisterPropertyDefinitionHandler ()
{
	...
	const auto result = ACAPI_Notify_RegisterEventHandler (GS::NewOwned<PropertyDefinitionEventHandler> (), *PropertyDefinitionEventHandlerId);
	if (result != NoError) {
...
GSErrCode	__ACENV_CALL RegisterPropertyEventHandlers (void)
{
...
GSErrCode	__ACENV_CALL UnRegisterPropertyEventHandlers (void)
{
	if (PropertyDefinitionEventHandlerId.HasValue ()) {
		ACAPI_Notify_UnregisterEventHandler (*PropertyDefinitionEventHandlerId);
		PropertyDefinitionEventHandlerId.Clear ();
	}

	if (PropertyGroupEventHandlerId.HasValue ()) {
		ACAPI_Notify_UnregisterEventHandler (*PropertyGroupEventHandlerId);
		PropertyGroupEventHandlerId.Clear ();
	}
