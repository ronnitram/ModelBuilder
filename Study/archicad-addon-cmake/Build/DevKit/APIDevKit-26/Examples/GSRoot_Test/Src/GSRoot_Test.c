...
static GSErrCode  	ArrayTest (bool timerOn)
{
	...
	for (i = 0; i < selectionInfo.sel_nElem; i++) {
					...
					for (i = 0; i < 20 && err1 == NoError; i++) {
						for (j = 1; j <= array.lastind && err1 == NoError; j++) {		// first record is not used
							if (err == NoError) {
								...
								((API_Element *) *array.arrhdl)[j].line.linePen.penIndex += 1;
								err1 = ACAPI_Element_Create (((API_Element *) *array.arrhdl) + j, nullptr);
								if (err1 != NoError)
...
static void		Level4 (Int32 what)
{
	switch (what) {
		...
		case 6:	{				// Generate memory leak
			char*	pChar = new char [256];	// buffer_overrun_reviewed_0
