...
static GSErrCode	AppendList (API_ListData *listdata, Int32 *index)
{
	...
	if (listdata == nullptr ||  index == nullptr || *index < 0)
...
static void		Search_ListData (API_ListData *param, Int32 *count, bool filter)
{
	...
	*count		= 0;
	...
	/* ACAPI_OldListing_Search fails if keyCode is an empty string */

	/* ------- Search it ------- */
	if (filter && (param->header.typeID == API_UnitID || param->header.typeID == API_KeyID ||
		(param->header.typeID == API_Obsolete_ComponentID && strlen (param->component.code) != 0) ||
	...
	/* ------ Search list ------ */
		...
		err = ACAPI_OldListing_GetNum (param->header.setIndex, param->header.typeID, &endIndex);
		if (err != NoError)
			return;
		...
		for (i = beginIndex; i <= endIndex; i++) {
			...
			if (filter) {
				if (listdata.header.typeID == API_Obsolete_ComponentID &&
...
static void ListDataDlg_Set (short 	dialId, DGUserData userData)
{
	...
	CHTruncate (temp.ToCStr (), keyStr, NAME_LEN2);
	temp = DGGetItemText (dialId, CodeEditTextItem);
	CHTruncate (temp.ToCStr (), codeStr, NAME_LEN2);

	switch (DGGetItemValLong (dialId, TypeIDPopupItem)) {

		case 1:		listdata.header.typeID = API_Obsolete_ComponentID;
	...
	switch (message) {
		case DG_MSG_INIT:
			{	API_ListDataSetType		listdataSet;
				...
				maxDBSet = ACAPI_OldListing_GetSetNum ();
...
static void		Do_ListData (void)
{
	...
	param.header.typeID   = API_Obsolete_ComponentID;
...
{
	ACAPI_MenuItem_RegisterMenu (32500, 0, MenuCode_Calculate, MenuFlag_Default);
		return NoError;
}		/* RegisterInterface */
...
GSErrCode	__ACENV_CALL Initialize	(void)
{
	...
	if (err != NoError)
		DBPrintf ("Listing Manager:: Initialize() ACAPI_MenuItem_InstallMenuHandler failed\n");
