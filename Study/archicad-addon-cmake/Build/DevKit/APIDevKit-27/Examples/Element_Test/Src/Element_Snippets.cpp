...
static void	__ACENV_CALL	RoomRedProc (const API_RoomReductionPolyType *roomRed)
{
	...
	DumpPolygon (actZone, APISolidLineAttributeIndex, 0.0,
...
void		Do_DumpZone (void)
{
	...
	for (k = 1; k <= nPoly; k++) {
		if (k == 1) {
			...
			DumpPolygon (&element, APISolidLineAttributeIndex, 2.0,
	...
	actZone = &element;
	err = ACAPI_Element_RoomReductions (&element.header.guid, RoomRedProc);
	if (err != NoError) {
...
void		Do_DumpWall (void)
{
	...
	if (!ClickAnElem ("Click a wall", API_WallID, nullptr, &element.header.type, &element.header.guid)) {
	...
	DumpPolygon (&element, APISolidLineAttributeIndex, 0.0,
...
void		Do_DumpBeam (void)
{
	...
	if (!ClickAnElem ("Click a beam", API_BeamID, nullptr, &element.header.type, &element.header.guid)) {
...
void		Do_DumpWindow (void)
{
	...
	if (!ClickAnElem ("Click a window", API_WindowID, nullptr, &element.header.type, &element.header.guid)) {
...
void		Do_DumpDoor (void)
{
	...
	if (!ClickAnElem ("Click a door", API_DoorID, nullptr, &element.header.type, &element.header.guid)) {
...
void		Do_DumpShell (void)
{
	...
	if (!ClickAnElem ("Click a shell", API_ShellID, nullptr, &element.header.type, &element.header.guid)) {
...
void		Do_NeigToCoord (void)
{
	...
	if (!ClickAnElem ("Click a point neig", API_ZombieElemID, &theNeig, &type, nullptr, &c2)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}

	WriteReport ("%s #%s", ElemID_To_Name (type).ToCStr (CC_UTF8).Get (),
	             		   APIGuidToString (theNeig.guid).ToCStr ().Get ());
	WriteReport ("  typ: %d", theNeig.neigID);
	WriteReport ("  inIndex: %d", theNeig.inIndex);

	err = ACAPI_Element_NeigToCoord (&theNeig, &coord);
	if (err == NoError) {
...
void		Do_GetBounds (void)
{
	...
	GSErrCode err = ACAPI_Element_CalcBounds (&element.header, &box);
	if (err != NoError) {
		ErrorBeep ("APIDb_CalcBoundsID", err);
		return;
	}

	WriteReport ("%s GUID:%s", ElemID_To_Name (element.header.type).ToCStr (CC_UTF8).Get (),
	...
	if (fabs (box.zMax - box.zMin) > EPS) {
		element2.header.type = API_WallID;
		...
		element2.header.layer = APIApplicationLayerAttributeIndex;
		...
		err = ACAPI_Grouping_Tool (elemGuids, APITool_SendBackward, nullptr);
		if (err != NoError)
			ErrorBeep ("ACAPI_Grouping_Tool (send backward)", err);
	} else {
		element2.header.type = API_LineID;
		element2.header.floorInd = element.header.floorInd;
		element2.header.layer = APIApplicationLayerAttributeIndex;
		element2.line.linePen.penIndex = 1;
		element2.line.ltypeInd = APISolidLineAttributeIndex;
...
void		Do_ShowSymbolHotspots (void)
{
	...
	if (!ClickAnElem ("Click a point neig", API_ZombieElemID, nullptr, &element.header.type, &element.header.guid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
...
void		Do_CalcQuantities (void)
{
	...
	if (!ClickAnElem ("Click an element to calculate quantities", API_ZombieElemID, nullptr, &type, &guid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
	...
	WriteReport ("%s GUID:%s", ElemID_To_Name (type).ToCStr (CC_UTF8).Get (),
	             			   APIGuidToString (element.header.guid).ToCStr ().Get ());
	...
	for (UInt32 i = 0; i < elemPartSurfaces.GetSize (); i++) {
		...
		surfaceQuantityString += "  MaterialIndex  : " + elemPartSurfaces[i].materialIndex.ToUniString () + GS::UniString (GS::EOL);
		surfaceQuantityString += "  BuildMatIndex  : " + elemPartSurfaces[i].buildMatIdx.ToUniString () + GS::UniString (GS::EOL);
...
void		Do_CalcComponentQuantities (void)
{
	...
	if (!ClickAnElem ("Click a point neig", API_ZombieElemID, nullptr, &type, &elemGuid)) {
		WriteReport_Alert ("No element was clicked");
		return;
	}
...
void		Do_SetToolBoxMode (void)
{
	...
	if (tboxInfo.type == API_ZombieElemID)
		...
		if (tboxInfo.type.variationID == APIVarId_Generic) {
			WriteReport ("Previous toolbox mode: %s", ElemID_To_Name (tboxInfo.type).ToCStr (CC_UTF8).Get ());
		} else {
			p = (char *) &tboxInfo.type.variationID;
			WriteReport ("Previous toolbox mode: %s  External \"%c%c%c%c\"",
			             ElemID_To_Name (tboxInfo.type).ToCStr (CC_UTF8).Get (),
			             p[0], p[1], p[2], p[3]);
		}
	}

	tboxInfo.type = API_ZombieElemID;
	if (ClickAnElem ("Click an element to set toolbox mode", API_ZombieElemID, nullptr, &tboxInfo.type, &guid)) {
	...
	err = ACAPI_Toolbox_SetToolBoxMode (&tboxInfo);
	if (err != NoError) {
	...
	if (tboxInfo.type == API_ZombieElemID)
		...
		if (tboxInfo.type.variationID == APIVarId_Generic) {
			WriteReport ("Current toolbox mode: %s", ElemID_To_Name (tboxInfo.type).ToCStr (CC_UTF8).Get ());
		} else {
			p = (char *) &tboxInfo.type.variationID;
...
void		Do_ChangeSubtypeSettings (void)
{
	...
	err = ACAPI_Toolbox_GetToolBoxInfo (&toolboxInfo);
	if (err != NoError) {
	...
	for (i = 0; i < toolboxInfo.nTools; i++) {
		if ((*toolboxInfo.data)[i].type.variationID == APIVarId_Generic)		// filter to subtype tools
			continue;

		// report the settings
		element = {};
		switch ((*toolboxInfo.data)[i].type.typeID) {
			case API_ObjectID:
						element.header.type = API_ObjectID;
						element.header.type.variationID = (*toolboxInfo.data)[i].type.variationID;
						break;
			case API_LampID:
						element.header.type = API_LampID;
						element.header.type.variationID = (*toolboxInfo.data)[i].type.variationID;
						break;
			case API_WindowID:
		...
		BNZeroMemory (&attribute, sizeof (API_Attribute));
		if (element.header.layer != APIInvalidAttributeIndex) {
			attribute.header.typeID = API_LayerID;
		...
		signature[4] = '\0';
		*((Int32 *) signature) = (*toolboxInfo.data)[i].type.variationID;
...
public:
	...
	{
		ACAPI_Grouping_GetConnectedElements (polyRoofGuid, API_SkylightID, &skylights);
	}
...
void	Do_SplitPolyRoof (void)
{
	...
	if (!ClickAnElem ("Click a roof to modify", API_RoofID, nullptr, &element.header.type, &element.header.guid)) {
...
static GSErrCode	OpenFile (const API_IOParams *ioParams)
{
	...
	try {
		gOutput = NewOwned<IO::File> (*ioParams->fileLoc, IO::File::Create);
		if (gOutput == nullptr)
			throw (1);
...
protected:
	...
	GSErrCode	WriteLinetype (const API_AttributeIndex& ltypeInd);
...
{
	char	buff [128];
	static API_AttributeIndex	lastLay = ACAPI_CreateAttributeIndex (- 1);
	static short lastPen = -2;

	if (lastLay != prim.header.layer) {
		sprintf (buff, "  Layer %s\n", prim.header.layer.ToUniString ().ToCStr ().Get ());
		gOutput->Write (buff);
...
GSErrCode	PrimElem::WriteLinetype (const API_AttributeIndex& ltypeInd)
{
	char	buff [64];
	static API_AttributeIndex	lastLtype = ACAPI_CreateAttributeIndex (- 1);

	if (lastLtype != ltypeInd) {
		sprintf (buff, "  Linetype %s\n", ltypeInd.ToUniString ().ToCStr ().Get ());
		gOutput->Write (buff);
		lastLtype = ltypeInd;
	}
...
public:
	...
	{
		ACAPI_Navigator_SetMasterLayoutOnLayout ((&databaseUnId));
	}
...
static GSErrCode DumpPrimitives ()
{
	...
	while ((ret = ACAPI_DrawOrder_DrawOrderGetNext ((Int32)(APIFilt_OnVisLayer | APIFilt_OnActFloor), &elem.header)) == NoError) {
		if (elem.header.type != API_HatchID && elem.header.type != API_DrawingID &&
			elem.header.type != API_ZoneID && elem.header.type != API_SlabID &&
			elem.header.type != API_MeshID && elem.header.type != API_RoofID &&
			elem.header.type != API_LineID && elem.header.type != API_PolyLineID &&
			elem.header.type != API_TextID)
			continue;
...
GSErrCode __ACENV_CALL Do_ExportPrimitives (const API_IOParams *ioParams)
{
	...
	if (err == NoError) {
		err = ACAPI_Database_GetCurrentDatabase (&origDBInfo);
		if (err == NoError && origDBInfo.masterLayoutUnId.elemSetId != APINULLGuid) {
			...
			err = ACAPI_Navigator_GetLayoutSets (&masterLayoutInfo, &masterDBInfo.databaseUnId);
	...
	if (err == NoError && needMaster && showMasterBelow) {
		...
		err = ACAPI_Database_ChangeCurrentDatabase (&masterDBInfo);
		...
		err = ACAPI_Database_ChangeCurrentDatabase (&origDBInfo);
	}
