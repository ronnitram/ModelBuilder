...
static void TestPropertiesOnElemList (const API_Guid& elemGuid)
{
	...
	ASSERT_NO_ERROR (ACAPI_Property_AddProperty (definition, elemList));
	...
	// ACAPI_Property_AddProperty can be used to add to the availability list of an existing property definition
	ASSERT_NO_ERROR (ACAPI_Property_AddProperty (definition, elemList));
	...
	// ACAPI_Property_DeleteProperty makes the property unavailable for the given elements
	ASSERT_NO_ERROR (ACAPI_Property_DeleteProperty (definition.guid, elemList));
	ASSERT_NO_ERROR (ACAPI_Property_GetPropertyDefinition (definition));
...
static void DefineNewIntProperty ()
{
	...
	API_PropertyDefinition definition;
	definition = PropertyTestHelpers::CreateExampleIntPropertyDefinition (group);
	// ACAPI_Property_AddProperty creates the property definition, if it doesn't exist yet
	ASSERT_NO_ERROR (ACAPI_Property_AddProperty (definition, PropertyTestHelpers::GetSelectedElements()));
	ASSERT (definition.guid != APINULLGuid);
...
static void DeleteIntegerPropeties ()
{
	...
	for (UIndex i = 0; i < definitions.GetSize (); ++i) {
		if (definitions[i].definitionType == API_PropertyCustomDefinitionType &&
			...
			definitions[i].valueType == API_PropertyIntegerValueType)
		{
			ASSERT_NO_ERROR (ACAPI_Property_DeleteProperty (definitions[i].guid, selectedElements));
		}
	}
}
