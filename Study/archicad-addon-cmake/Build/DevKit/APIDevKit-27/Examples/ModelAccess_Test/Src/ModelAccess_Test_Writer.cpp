...
WRL::MAWriter::MAWriter (const IO::Location& ioParam_fileLoc, ModelerAPI::Model* maModel):
	outputFile (nullptr),
	model (nullptr),
	definedMaterials (nullptr),
	nMaterials (0)
{
	if (DBERROR (maModel == nullptr))
		throw GS::NullPointerException ();

	model = maModel;
	IO::Location			fileLoc;
	fileLoc = ioParam_fileLoc;
	nMaterials = model->GetMaterialCount ();
...
//------------------------------------------------------------------------------
void		WRL::MAWriter::WriteDefMaterial (const ModelerAPI::AttributeIndex& iumat)
{
	char	buffer[256];

	sprintf (buffer, "DEF Mat_%ld Material {\n", (GS::LongForStdio) iumat.GetIndex ());
	WriteBuffer (buffer);
...
void		WRL::MAWriter::WriteTexCoordAdd (bool isLast, ModelerAPI::TextureCoordinate* uvCoord)
{
	...
	if (fabs (uvCoord->u) < EPS) uvCoord->u = 0.0;
	if (fabs (uvCoord->v) < EPS) uvCoord->u = 0.0;
...
//------------------------------------------------------------------------------
void		DumpMaterialRenderingSettings (const ModelerAPI::Material& material)
{
	ModelerAPI::ParameterList parameters;
	material.GetExtraParameters (&parameters);
	for (Int32 ii = 1; ii <= parameters.GetParameterCount (); ++ii) {
...
//------------------------------------------------------------------------------
void		WRL::MAWriter::WriteAppearance (const ModelerAPI::AttributeIndex& iumat, char* textureName)
{
	char	buffer[256];
	...
	if (definedMaterials[iumat.GetIndex () - 1]) {
		sprintf (buffer, "\t\t\t\tmaterial USE Mat_%ld\n", (GS::LongForStdio) iumat.GetIndex ());
		WriteBuffer (buffer);
	} else {
		sprintf (buffer, "\t\t\t\tmaterial %ld", (GS::LongForStdio) iumat.GetIndex ());
		WriteBuffer (buffer);

		WriteDefMaterial (iumat);
		definedMaterials[iumat.GetIndex () - 1] = true;
	}
