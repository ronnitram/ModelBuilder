...
WRL::MAExporter::MAExporter (ModelerAPI::Model* newModel, const API_IOParams* ioParams) :
	...
	writer (nullptr)
{
	if (DBERROR (newModel == nullptr))
		throw GS::NullPointerException ();
	model = newModel;

	if (DBERROR (ioParams == nullptr))
		throw GS::NullPointerException ();
...
{
	Int32	nTexture = model->GetTextureCount ();
	if (nTexture <= 0)
		return;
	...
	for (Int32 i = 2; i <= nTexture; i++) {	// the first texture is the background picture
		ModelerAPI::AttributeIndex iText (ModelerAPI::AttributeIndex::TextureIndex, i);
		CreateTextureFile (iText, fileName);
...
//------------------------------------------------------------------------------
void		WRL::MAExporter::CreateTextureFile (const ModelerAPI::AttributeIndex& textureIndex, char* fullPathName)
{
	...
	if (pixMapHdl == nullptr) {
		delete[] pixelMap;
		throw GS::OutOfMemoryException ();
	}

	GSPtr pixMapPtr = GXGetGSPixMapBaseAddr (pixMapHdl);
	if (DBERROR (pixMapPtr == nullptr)) {
	...
	if (name[0] == '\0')
		std::snprintf (fileName, sizeof (fileName), "%s%ld.jpg", localizedStrings[WRL::TextureName].ToCStr ().Get (), (GS::LongForStdio) textureIndex.GetIndex ());
	else
		std::snprintf (fileName, sizeof (fileName), "%s.jpg", name);
	...
	GSErrCode err = aimage.WriteToFile (loc, FTM::FileTypeManager::SearchForMime("image/jpeg", nullptr), 24);
...
{
	if (DBERROR (axis == nullptr || angle == nullptr))
		throw GS::NullPointerException ();
	...
	TRANMAT	atran;
	Vector3d vec4 (0.0, 0.0, 0.0);
	atran.tmx = Geometry::Matrix34::CreateFromColVectors (right, top, -dir, vec4);

	// calculation of the angle and the axis
	sinangle = sqrt ((right.y - top.x) * (right.y - top.x) / 4 +
		(-dir.x - right.z) * (-dir.x - right.z) / 4 +
		(top.z + dir.y) * (top.z + dir.y) / 4);
	cosangle = (right.x + top.y + -dir.z - 1) / 2;
	...
	*angle = 0.0;
	...
	if (*angle > EPS) {
		...
		q = (right.x - cosangle) / p;	// theoretically q >= 0
...
{
	if (DBERROR (minCoord == nullptr || maxCoord == nullptr))
		throw GS::NullPointerException ();
	...
	Int32						nElements = model->GetElementCount ();
	...
	for (iElement = 1; iElement <= nElements; iElement++) {
		...
		nBody = element.GetTessellatedBodyCount ();
		...
		for (iBody = 1; iBody <= nBody; iBody++) {
			...
			for (iPgon = 1; iPgon <= nPoly; iPgon++) {
				try {
					...
					nConvexPolygons = polygon.GetConvexPolygonCount ();
					...
					for (iConvexPolygons = 1; iConvexPolygons <= nConvexPolygons; iConvexPolygons++) {
						...
						nPedge = convexPolygon.GetVertexCount ();
...
void		WRL::MAExporter::ExportCameraFromSaveAs3D  (void)
{
	...
	ACAPI_View_Get3DProjectionSets (&projSets);
...
{
	double					intensity;
	Int32					elemNum = model->GetElementCount ();
...
// Export a body
//------------------------------------------------------------------------------
void		WRL::MAExporter::ExportBody (ModelerAPI::MeshBody& body)
{
	Int32	nPgon = body.GetPolygonCount ();
	if (nPgon <= 0)
		return;

	Int32	nVertex = body.GetVertexCount ();
	if (nVertex <= 0)
		return;

	bool	smooth = body.IsVisibleIfContour ();
	...
	for (iPgon = 1; iPgon <= nPgon; iPgon++) {
		...
		try {
			nConvexPgon = polygon.GetConvexPolygonCount ();
		...
		if (iText.IsValid () && iText.GetIndex () > 1) {
			CHTruncate (textureNames[iText.GetIndex () - 2].ToCStr ().Get (), textureName, WRL::MaxStringLength);
		}

		ModelerAPI::AttributeIndex iMat (ModelerAPI::AttributeIndex::MaterialIndex);
		polygon.GetMaterialIndex (iMat);
		writer->WriteAppearance (iMat, textureName);
...
void		WRL::MAExporter::ExportBodies (void)
{
	...
	Int32	phaseNum = 1;
	ACAPI_ProcessWindow_InitProcessWindow (&localizedStrings[WRL::SavingFile], &phaseNum);
	ACAPI_ProcessWindow_SetNextProcessPhase (nullptr, &nElements);

	try {
		for (Int32 iElement = 1; iElement <= nElements; iElement++) {
			model->GetElement (iElement, &element);
			nBodies = element.GetTessellatedBodyCount ();
			...
			ACAPI_ProcessWindow_SetProcessValue (&iElement);
			if (iElement % 1000 == 0) {
