...
static void		Do_Switch3DCutaway ()
{
	...
	GSErrCode err = ACAPI_CuttingPlane_Is3DCutawayEnabled (&status);
	if (err != NoError) {
		ACAPI_WriteReport ("APIIo_Is3DCutawayEnabledID failed!", true);
	} else {
		status = !status;
		err = ACAPI_CuttingPlane_Set3DCutawayStatus (&status);
		if (err != NoError)
...
static void		Do_SwitchCuttingPlanes ()
{
	...
	GSErrCode err = ACAPI_CuttingPlane_IsCuttingPlanesVisible (&status);
	if (err != NoError) {
		ACAPI_WriteReport ("APIIo_IsCuttingPlanesVisibleID failed!", true);
	} else {
		status = !status;
		err = ACAPI_CuttingPlane_SetCuttingPlanesVisibility (&status);
		if (err != NoError)
